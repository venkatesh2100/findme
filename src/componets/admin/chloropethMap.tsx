...
export default function ChoroplethMap() {
  ...
  const [stateData, setStateData] = useState<Map<string, StateData>>(
    new Map(STATES_DATA.map((d) => [d.code, d]))
  );
  ...
  useEffect(() => {
    ...
    const maxValue = Math.max(
      ...Array.from(stateData.values()).map((d) => d.value)
    );
    const colorScale = d3
      .scaleLinear<string>()
      .domain([0, maxValue])
      ...
      group
        ...
        .data(
          states.features.filter(
            (d) => !filter || filter(d.properties?.name || "")
          )
        )
        .enter()
        .append("path")
    ...
    const handleResize = () => {
      ...
      mainGroup
        .selectAll("path")
        .attr("d", (d) => mainPath(d as d3.GeoPermissibleObjects) || "");
      alaskaGroup
        .selectAll("path")
        .attr("d", (d) => alaskaPath(d as d3.GeoPermissibleObjects) || "");
      hawaiiGroup
        .selectAll("path")
        .attr("d", (d) => hawaiiPath(d as d3.GeoPermissibleObjects) || "");
    };
